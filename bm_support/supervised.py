...
from datahelpers.dftools import (
    select_appropriate_datapoints,
    dict_to_array,
    accumulate_dicts,
    add_column_from_file,
)
...
def get_dataset(
    ...
    **kwargs
    ...
    if hash_int:
        ...
        fname = "data_batches_{0}_v_{1}_c_{2}_m_{3}_n_{4}_a_{5}_b_{6}.pgz".format(
...
def generate_samples(
    ...
    if load_batches:
        ...
        if hash_int:
            ...
            fname = "df_{0}_v_{1}_c_{2}_m_{3}_n_{4}_a_{5}_b_{6}.pgz".format(
                origin, version, data_columns, batchsize, cutoff_len, lo, hi
            )
        if fname[-3:] == "pgz":
    ...
    if verbose:
        print(
            "df size {0}, df unique [up, dn, pm] {1}".format(
                df_claims.shape[0], df_claims.drop_duplicates([up, dn, pm]).shape[0]
            )
        )
    ...
    dfe = (
        df_exp_cut.groupby(agg_columns)
        .apply(
            lambda x: pd.Series(
                [np.mean(x["score"]), np.std(x["score"])], index=["mean", "std"]
            )
        )
        .reset_index()
    )
    ...
    if verbose:
        print(
            "min score {0}, max score {1}".format(
                df_exp_cut["score"].min(), df_exp_cut["score"].max()
            )
        )
    ...
    if verbose:
        print(
            "experimental statements after agg and merge rows: {0}".format(
                dfe2.shape[0]
            )
        )
    ...
    if (
        not load_batches
        and hash_int
        and up in df_claims.columns
        and dn in df_claims.columns
    ):
        merge_on = o_columns
    ...
    if verbose:
        print("###")
        print(
            "claims: before: {0} after: {1}  fraction: {2:.4f}".format(
                df_claims.shape[0], dft.shape[0], dft.shape[0] / df_claims.shape[0]
            )
        )
        n_ints = df_claims.drop_duplicates([up, dn]).shape[0]
        n_ints_after = dft.drop_duplicates([up, dn]).shape[0]
        print(
            "interactions: before: {0} after: {1}  fraction: {2:.4f}".format(
                n_ints, n_ints_after, n_ints_after / n_ints
            )
        )
    ...
    for ty in types_comm:
        ...
        for fn, cn in zip(fnames, cnames):
            dfc = pd.read_csv(join(fpath_comm, fn), compression="gzip", index_col=0)
            comm_ids = set(dfc.index)
            if verbose:
                print(
                    "{0} {1}. |ids_merge| : {2}. |ids_comm| {3}. |ids_merge - ids_comm| {4}.".format(
                        fn, cn, len(all_ids), len(comm_ids), len(all_ids - comm_ids)
                    )
                )
            vc = dfc.groupby("comm_id").apply(lambda x: x.shape[0])
            dfc2 = dfc.merge(
                pd.DataFrame(vc), left_on="comm_id", right_index=True
            ).rename(columns={0: "csize"})
            dfc2_up = dfc2.rename(
                columns=dict([(c, cn + "_" + c + "_up") for c in dfc2.columns])
            )
            dfc2_dn = dfc2.rename(
                columns=dict([(c, cn + "_" + c + "_dn") for c in dfc2.columns])
            )
            ...
            up_dns[cn + "_same_comm"] = (
                up_dns[cn + "_comm_id_up"] == up_dns[cn + "_comm_id_dn"]
            )
            up_dns[cn + "_eff_comm_size"] = (
                up_dns[cn + "_csize_up"] * up_dns[cn + "_csize_dn"]
            ) ** 0.5
    ...
    up_dns_ye = dft.drop_duplicates([up, dn, ye])[[up, dn, ye]].sort_values(
        [up, dn, ye]
    )
    up_dns_ye_acc = dft.drop_duplicates([up, dn, ye])[[up, dn, ye]].sort_values(
        [up, dn, ye]
    )
    ...
    for ff, cn in list(zip(fnames, cnames)):
        ...
        for k in keys[:]:
            ...
            ud_cur[cn + "_same_comm"] = (
                ud_cur[cn + "_comm_id_up"] == ud_cur[cn + "_comm_id_dn"]
            )
            ud_cur[cn + "_eff_comm_size"] = (
                ud_cur[cn + "_csize_up"] * ud_cur[cn + "_csize_dn"]
            ) ** 0.5
    ...
    for mt in metric_types:
        ...
        for ms in metric_sources2:
            ...
            df_att = pd.read_csv(
                expanduser("{0}{1}_metric_{2}.csv.gz".format(fpath, mt, ms))
            )
...
def simple_oversample(df, statify_column, rns, ratios=None, verbose=False):
    ...
    indices = [
        rns.choice(actual, new_size, replace=True)
        for actual, new_size in zip(sizes[1:], new_sizes)
    ]
    # print([len(ii) for ii in indices])
    subdfs = [df.loc[masks[0]]] + [
        df.loc[m].iloc[ii] for m, ii in zip(masks[1:], indices)
    ]
    dfr = pd.concat(subdfs)
    return dfr


def smart_stratify_df(
    df, column, size=500, ratios=None, replacement=False, seed=17, verbose=False
):
    vc = df[column].value_counts()
    if verbose:
        print(vc)
...
def logreg_analysis(
    ...
    logreg = LogisticRegression(C=1.0 / regularizer, penalty="l1", fit_intercept=False)
    logreg = logreg.fit(X_train, y_train)
    print("### lr intercept and coefs")

    rep = list(
        zip(covariate_columns, list(map(lambda x: "{:.3f}".format(x), logreg.coef_[0])))
    )
    print(rep)
    ...
    modelCV = LogisticRegression(C=1.0 / regularizer, penalty="l1", fit_intercept=False)
    scoring = "accuracy"
    results = model_selection.cross_val_score(
        modelCV, X_train, y_train, cv=kfold, scoring=scoring
    )
    print(
        "{0}-fold cross validation average accuracy: {1:.3f}".format(
            nfolds, results.mean()
        )
    )
...
def rf_study(
    ...
    rf = RandomForestClassifier(
        n_trees, max_depth=depth, random_state=seed, class_weight=class_weight
    )
    ...
    if n_states > 2:
        ...
        auroc = [
            roc_auc_score(y_, proba_)
            for proba_, y_ in zip(positive_proba.T, y_test_binary.T)
        ]
    else:
    ...
    if n_states > 2:
        ...
        for cs, auc, k in zip(coords, auroc, range(n_states)):
            ...
            ax.plot(
                fpr,
                tpr,
                label="Random Forest: dist {0} area = {1:.3f}".format(distance, auc),
            )
    else:
...
def train_forest(dfw, feature_columns, y_column, seed, n_trees=10, mode_scores=None):
    ...
    df_train, df_test = train_test_split(
        dfw, test_size=0.3, random_state=seed, stratify=dfw[y_column]
    )
...
def train_massif(
    ...
    probs = [
        clean_zeros(rfm.predict_proba(X_test), 1e-2)[np.newaxis, ...] for rfm in massif
    ]
        ...
        if n_states > 2:
            ...
            for cs, auc, k in zip(coords, auroc, range(n_states)):
                ...
                ax.plot(
                    fpr,
                    tpr,
                    label="Random Forest: dist {0} "
                    "area = {1:.3f}".format(distance, auc),
                )
        else:
            fpr, tpr, thresholds = roc_curve(y_test, positive_proba)
            ax.plot(
                fpr, tpr, label="Random Forest: area = {0:.3f}".format(report["auroc"])
            )
...
def train_massif_lr_clean(
    df_train, feature_columns, y_column, seed_=11, n_throws=10, n_trees=10, ratios=None
):
    massif = []
...
def plot_importances(
    ...
    if sort_them:
        if isinstance(sort_them, dict):
            indices = np.argsort(
                [
                    sort_them[c] if c in sort_them.keys() else len(sort_them) + 10
    ...
    colors = [
        colors_dict[0] if x > 0 else colors_dict[1] for x in importances2[sorted_ix]
    ]
    imp_ccs2 = [imp_ccs[i] for i in sorted_ix]
    error_kw = dict(lw=5, capthick=2)
    plt.bar(
        range(len(importances2)),
        importances2[sorted_ix],
        color=colors,
        yerr=stds2[sorted_ix],
        align="center",
        alpha=0.6,
        error_kw=error_kw,
    )
...
def plot_lr_coeffs_with_penalty(
    alphas,
    coeff_dict,
    covariate_names,
    fname=None,
    ...
    ax.set_xlim(
        [
            1e1 ** np.floor(np.log10(np.min(alphas))),
    ...
    if logy:
        ...
        ax.set_ylim(
            [
                1e1 ** np.floor(np.log10(np.min(np.abs(arr[np.nonzero(arr)])))),
...
def lr_study(
    ...
    if fname or show:
        ...
        ax.plot(
            fpr,
            tpr,
            label="Logistic Regression: area = {0:.3f}".format(report["auroc"]),
        )
        ax.plot([0, 1], [0, 1], "r--")
        ax.set_xlim([0.0, 1.05])
        ax.set_ylim([0.0, 1.05])
...
def lr_study_one_sample(X_train, X_test, y_train, y_test, covariate_columns=[], seed=0):
    ...
    for coeff in reg_coeffs:
        report = lr_study(
            X_train, X_test, y_train, y_test, covariate_columns, seed, coeff
        )
...
def run_n_lr_studies(
    df, feature_columns, y_column, n_throws=50, seed=0, ratios=(1.0, 2.0)
):
    np.random.seed(seed)
    ...
    for seed in seeds:
        df_train, df_test = train_test_split(
            df, test_size=0.3, random_state=seed, stratify=df[y_column]
        )
        dd = simple_stratify(df_train, y_column, seed, ratios=ratios)
        X_train, y_train = dd[feature_columns], dd[y_column]
        X_test, y_test = df_test[feature_columns], df_test[y_column]
        ...
        r["seed"] = seed
        r["corr_train"] = (
            dd[feature_columns + [y_column]].corr().iloc[-1].iloc[:-1].values
        )
        r["corr_test"] = (
            df_test[feature_columns + [y_column]].corr().iloc[-1].iloc[:-1].values
        )
...
def reduce_report(report):
    ...
    for k in kkeys:
        if isinstance(r_sample[k], (int, float)):
            ...
            final_report[k] = (
                np.array([r["feature_ranking"] for r in report]).mean(axis=0),
...
def parse_experiments_reports(reports):
    ...
    for k, dd in reports:
        ...
        for kk in dd.keys():
            ...
            if (
                isinstance(dd[kk], tuple)
                and (len(dd[kk]) == 2)
                and isinstance(dd[kk][0], float)
            ):
...
def std_over_samples(lengths, means, stds):
    ...
    total_mean = (
        np.sum(list(map(lambda x: x[0] * x[1], zip(lengths, means)))) / total_length
    )
    total_num = np.sum(
        list(map(lambda x: x[0] * (x[1] ** 2 + x[2] ** 2), zip(lengths, means, stds)))
    )
...
def invert_bayes(df_input, clf, feature_columns, p_min=1e-2, verbose=False, debug=None):
    ...
    if isinstance(debug, np.ndarray):
        ...
        if isinstance(clf, list):
            ...
            probs = [
                rfm.predict_proba(df[feature_columns])[np.newaxis, ...] for rfm in clf
            ]
            arr_probs = np.concatenate(probs, axis=0)
...
def aggregate_over_claims(df, barrier):
    ...
    p_agg4 = p_agg4.merge(
        pd.DataFrame(df.drop_duplicates(ni)[[ni, "qcdf_exp"]]),
        how="left",
        left_index=True,
        right_on=ni,
    )
...
def aggregate_over_claims_comm(
    dfn,
    dft_comm,
    groupby_columns=(up, dn),
    groupby_columns2=(up, dn, pm),
    groupby_columns3=(up, dn, "rcommid"),
):
    pes = ["pe_{0}".format(j) for j in range(3)]
    dfn2 = dfn.merge(dft_comm, how="inner", on=groupby_columns2)
    dfn3 = (
        dfn2.groupby(groupby_columns3)
        .apply(lambda x: np.mean(np.log(x[pes]), axis=0))
        .reset_index()
    )
...
def kmeans_cluster(
    data, n_classes=2, seed=11, tol=1e-6, verbose=False, return_flags=True
):
    ...
    mus = [
        np.array([rns.uniform(x, y) for x, y in zip(mins, maxs)])
...
def identify_intracluster_distances(
    data, n_classes=2, seed=11, tol=1e-6, verbose=False
):
    if n_classes > 1:
        ...
        if n_classes > len(unis):
            print(
                "Warning {0} classes predict {1} unique members".format(
                    n_classes, len(np.unique(args))
                )
            )
...
def yield_candidates(
    corrs, pi_columns, target_column=dist, critical_size=3, mode="both", verbose=False
):
    """
    ...
    if mode == "anti" or mode == "both":
        ...
        anti_corr = corrs[candidates].apply(
            lambda x: np.sum((x[pi_columns] + 1.0) ** 2)
        )
    ...
    if mode == "both":
        ...
        intersections = np.array(
            [
                len(set(candidates_anti_corr[:k]) & set(candidates_target_corr[-k:]))
                for k in range(len(candidates_anti_corr))
            ]
        )
        critical_index = np.argmax(intersections >= critical_size)
        candidates_batch = list(
            set(candidates_anti_corr[:critical_index])
...
def engine(
    ...
    for k in range(max_iterations):
        ...
        candidates = yield_candidates(
            corrs, current_features, target_column, critical_size, mode
        )
        if verbose:
...
def logreg_driver(origin, version, batchsize, cutoff_len, a, b, hash_int, max_depth):
    ...
    df_reports2 = df_reports.reset_index()[
        ["accuracy", "precision", "recall", "corr_train_pred", "f1", "auroc", "index"]
    ].sort_values("f1", ascending=False)

    fpath = expanduser(
        "~/data/kl/reports/report_logreg_{0}_{1}_{2}.csv".format(
            origin, version, hash_int
        )
    )
...
def get_corrs(
    ...
    if verbose:
        print(
            "number of rows {0}, which is a fraction {1:.3f} of the original".format(
                df_.shape[0], df_.shape[0] / df.shape[0]
            )
        )
    ...
    if isinstance(covariate_columns, dict):
        ...
        covariate_columns = list(
            [x for sublist in covariate_columns.values() for x in sublist]
        )
        dict_flag = True
    else:
    ...
    if individual_na:
        ...
        corr_abs_thr = (
            corr_df[corr_df.abs() > threshold].abs().sort_values(ascending=False)
        )
        ...
        if dict_flag:
            for k, v in covariate_columns_dict.items():
                ...
                if len(cur_cols) > 1:
                    candidates = (
                        corr_df.loc[cur_cols, target_column]
        ...
        corr_abs_thr = corr_df_abs.loc[
            list(set(reduced_columns) & above_thr), target_column
        ].sort_values(ascending=False)
        corr_df_thr = corr_df.loc[
            list(set(reduced_columns) & above_thr), target_column
        ].sort_values(ascending=False)
...
def trim_corrs_by_family(cr, feature_dict_inv):
    ...
    if cr.shape[0] > 0:
        ...
        cr_df_cut_pos = (
            ...
            .groupby("family")
            .apply(
                lambda x: x.iloc[np.argmax(x[cr_name].values)].append(
        ...
        cr_df_cut_neg = (
            ...
            .groupby("family")
            .apply(
                lambda x: x.iloc[np.argmin(x[cr_name].values)].append(
                    pd.Series([x.shape[0]], ["len_members"])
                )
            )
        )
        if cr_df_cut_neg.shape[0] > 0 or cr_df_cut_pos.shape[0] > 0:
            cr_res = (
                pd.concat([cr_df_cut_pos, cr_df_cut_neg])
                .set_index("index")
                .sort_values(cr_name)
            )
            unique_family_size = cr_res.drop_duplicates("family").shape[0]
            if unique_family_size != cr_res.shape[0]:
                confused_families = cr_res.groupby("family").apply(
                    lambda x: x.shape[0] > 1
                )
                confused_families_mask = cr_res["family"].isin(
                    confused_families[confused_families].index
                )
                print(
                    "Warning, a family of features has correlation of different signs!"
                )
                print(
                    "size of extreme corrs dataset: {0}, size of unique families dataset {1}".format(
                        unique_family_size, cr_res.shape[0]
                    )
                )
...
def select_features_dict(
    ...
    while len(chosen_features) <= max_features_consider and feature_dict_dyn:
        ...
        if len(chosen_metrics) > 0:
            potential_improvement = np.sign(chosen_metrics[-1]) * (
                1 - chosen_metrics[-1] / scalar_metrics[add_index]
            )
            if verbose:
                print(
                    "Fractional potential improvement: {0:.3f}, abs value: {1:.3f}".format(
                        potential_improvement, scalar_metrics[add_index]
                    )
                )
        ...
        if verbose:
            print(
                "nf: {0} cfeature: {1} metric: {2:.3f} metric_improv: {3:.2f} %".format(
                    len(cur_features),
                    (current_feature[:47] + "...").ljust(50),
                    chosen_metrics[-1],
                    100 * potential_improvement,
                )
            )

        if len(feature_dict_dyn[feature_group]) <= len(feature_dict[feature_group]) - (
            max_feat_per_family - 1
        ):
            if verbose:
                str_rep = "in dyn dict {0} in tot dict {1}".format(
                    len(feature_dict_dyn[feature_group]),
...
def report_metrics(
    ...
    report = report_metrics_(
        y_test, y_pred, mode_scores, metric_uniform_exponent, metric_mode, problem_type
    )
...
def report_metrics_(
    ...
    if problem_type == "class":
        ...
        report["accuracy"]["balanced"] = balanced_accuracy_score(
            y_test, y_pred, adjusted=True
        )

        report["vector"] = dict()
        report["vector"]["precision"] = precision_score(
            y_test, y_pred, average=mode_scores
        )
        ...
        report["exponent"] = {
            k: np.sum(v ** metric_uniform_exponent) / nclasses
