...
def study_sample(seed, dfw, dist, feature_dict,
    ...
    for ii in enums:
        ...
        rmetrics = report_metrics(model_, df_valid[cfeatures], df_valid[dist],
                                  mode_scores=mode_scores,
                                  metric_uniform_exponent=metric_uniform_exponent,
                                  metric_mode=metric_mode)
    ...
    metrics_vec = [x['validation_metrics'] for x in meta_agg]

    if metric_mode == 'accuracy':
        vmetrics = [x['accuracy']['balanced'] for x in metrics_vec]
    else:
        vmetrics = [x['macro'][metric_mode] for x in metrics_vec]

    index_best_run = np.argmax(vmetrics)
    ...
    report_dict['max_scalar_mm'] = np.max(vmetrics)
    report_dict['vmetrics'] = metrics_vec
