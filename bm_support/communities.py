...
def pick_community_by_rank(df, window=None, comm_rank=1, min_comms=2):
    ...
    rank_commids = (
        df2.groupby([up, dn])
        .apply(lambda x: x.loc[x[rank_col] == comm_rank, commid_col])
        .reset_index()
    )
...
def pick_min_communities(df, window=None, min_comm_sizes=2):
    ...
    dfr = df.copy()
    flags = dfr.groupby([up, dn]).apply(
        lambda x: sorted(x[rank_col].unique())[:min_comm_sizes] == min_ranks_composition
    )
