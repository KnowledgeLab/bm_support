...
def gap_stat(data, nc, nsamples=10, seed=17):
    ...
    random_samples = [
        rns.uniform(size=nsize).reshape(data.shape) * (maxs - mins) + mins
    ...
    log_wks = log(wks)
    sk = std(log_wks) * sqrt(1.0 + 1.0 / nsamples)
...
def choose_nc(data, nc_max=3, verbose=False):
    if nc_max > 1:
        ...
        raise ValueError(
            "nc_max should be greater than 1, value {0} supplied instead".format(nc_max)
        )
    return nc_opt
