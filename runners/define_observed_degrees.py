...
dfye = pd.read_csv(expanduser('~/data/kl/comms/interaction_network/updn_years2.csv.gz'), index_col=0)
...
for k in keys:
    ...
    g, ws, inv_conversion_map = prepare_graph_from_df(df, ttargs['file_format'], ttargs['directed'],
                                                      ttargs['percentile_value'])
...
dfd = pd.DataFrame(dgs, columns=['up', 'dn', 'year',
