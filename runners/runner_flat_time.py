...
def generate_fnames(j):
    return {'figname_prefix': 'model_notau_1K_{0}'.format(j),
            'tracename_prefix': 'trace_model_notau_1K_{0}'.format(j)}
...
with gzip.open(join('./../../../reports/',
                    '{0}.pgz'.format('report_identity_ai_hiai_pos_1000')), 'wb') as fp:
