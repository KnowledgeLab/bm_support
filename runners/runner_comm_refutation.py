...
dfr = derive_refutes_community_features(dfps, fpath, windows=[2, 3, None],
                                        metric_sources=('affiliations', 'authors', 'past'),
                                        work_var=ps, verbose=True)
dfr.to_csv(expanduser('~/data/kl/comm_metrics/{0}_comm_window_ave.csv.gz'.format(ps)), compression='gzip')
