...
def main(head, window_sizes, flag_dict, metric_type, verbosity=False):
    ...
    if flag_dict['support']:
        ...
        if head < 0:
            dft.to_csv(expanduser('~/data/wos/comm_metrics/support_metric_{0}.csv.gz'.format(metric_type)),
                       compression='gzip')
        else:
            print(dft.head())
            dft.to_csv(expanduser('~/data/wos/comm_metrics/support_metric_{0}_tmp2.csv.gz'.format(metric_type)),
                       compression='gzip')
    ...
    if flag_dict['affinity']:
        ...
        for c in dft.columns:
            print('{0} : {1:.2f} %'.format(c, 100 * sum(dft[c] != 0) / dft.shape[0]))
        if head < 0:
            dft.to_csv(expanduser('~/data/wos/comm_metrics/affinity_metric_{0}.csv.gz'.format(metric_type)),
                       compression='gzip')
        else:
            print(dft.head())
            dft.to_csv(expanduser('~/data/wos/comm_metrics/affinity_metric_{0}_tmp2.csv.gz'.format(metric_type)),
                       compression='gzip')
    ...
    if flag_dict['mod']:
        ...
        if head < 0:
            dft.to_csv(expanduser('~/data/wos/comm_metrics/modularity_metric_{0}.csv.gz'.format(metric_type)),
                       compression='gzip')
        else:
            print(dft.head())
            dft.to_csv(expanduser('~/data/wos/comm_metrics/modularity_metric_{0}_tmp2.csv.gz'.format(metric_type)),
                       compression='gzip')
