...
def sample_one_batch_per_interaction(df, seed=13):
    ...
    dfupdnye = dft.groupby([up, dn]).apply(lambda x:
        x[ye].iloc[rns.randint(x.shape[0])]).reset_index().rename(columns={0: ye})
    print(dfupdnye.head())
...
def aggregate_comm(dfw, comm_col):
    pi2 = dfw.groupby([up, dn, ye,comm_col]).apply(lambda x: pd.Series([x[cexp].unique()[0],
                                                                        x['pi_est'].mean()]))
    pi3 = pi2.groupby(level=[0, 1, 2]).apply(lambda x: pd.Series([x[0].unique()[0], x[1].mean()]))
    pi4 = pi3.groupby(level=[0, 1]).apply(lambda x: pd.Series([x[0].unique()[0], x[1].mean()]))
    return pi4
...
def compute_pis(df_, y_pred, eps=1e-6):
    ...
    y_fit_trans = (y_fit_trans - eps)/(1. - 2*eps)
    df_['dpred'] = y_fit_trans

    # simple aggregation
    df_['pi_est'] = df_.apply(lambda x: 1-x['dpred'] if x[ps] else x['dpred'], axis=1)

    pi_comp = df_.groupby([up, dn]).apply(lambda x: pd.Series([x[cexp].unique()[0], x['pi_est'].mean()]))
...
mns = dft.groupby([up, dn, ye]).apply(lambda x: pd.Series(x[rdist].mean(),
                                                          index=['mean_rdist']))
...
change_flags = mns_long.groupby([up, dn]).apply(lambda x: find_transition(x)).reset_index()

dfn = pd.merge(dft, change_flags, on=[up, dn, ye], how='right').sort_values([up, dn, ye])
...
df_rsk = pd.DataFrame(np.array(r_sk).T, columns=['pval', 'coeff', 'sigma'],
                      index=['intercept'] + nzero_features).sort_values('pval')

print('p-value ')
print(df_rsk)
print('mse: {0:.3f}, r2 {1:.3f}'.format(mean_squared_error(y, y_fit), r2_score(y, y_fit)))
...
print(pd.Series(rf.feature_importances_, index=nzero_features).sort_values(ascending=False))
...
pi_comp = dfw.groupby([up, dn]).apply(lambda x: pd.Series([x[cexp].unique()[0], x['pi_est'].mean()]))
mse_ref = mean_squared_error(pi_comp[0].values, pi_comp[1].values)
r2_ref = r2_score(pi_comp[0].values, pi_comp[1].values)
...
print('accuracy interaction: {0}'.format((sum((pi_comp[0]-0.5)*(pi_comp[1]-0.5) > 0)/pi_comp.shape[0])))
