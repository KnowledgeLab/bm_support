...
import pickle
import gzip
from bm_support.supervised_aux import run_model_splits
...
cfeatures = (set(feat_selector['claim']) | set(feat_selector['batch'])) - (set(gw_excl) | set(excl_set))
...
reports = run_model_splits(df, cfeatures, rns, target, mode=mode, n_splits=3, clf_parameters=complexity_dict)
j0, dfs_, clf, metrics_dict, coefficients, cfeatures = reports[0]
