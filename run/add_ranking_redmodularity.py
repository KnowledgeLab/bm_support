...
for ms in metric_sources:
    ...
    for w in windows:
        print(f" *** : {ms} {w}")
        df = pd.read_csv(
            expanduser(f"{fpath}{mt}_metric_{ms}_w{w}.csv.gz"), index_col=0
        )
        ...
        if head > 0:
            df = df.head(head)
        df2 = df.groupby([up, dn, "ylook"], as_index=False, group_keys=False).apply(
            lambda item: item.drop_duplicates(id_col)[
                [up, dn, "ylook", relsize_col, id_col]
            ]
        )

        df3 = (
            df2.groupby([up, dn, "ylook"])
            .apply(
                lambda x: pd.DataFrame(
                    rank_degenerate_values(x[id_col], x[relsize_col]),
                    columns=[id_col, rank_col],
                )
            )
            .reset_index()
        )
    ...
    if head < 0:
        dft.to_csv(
            expanduser(f"~/data/wos/comm_metrics/{mt}_metric_{ms}_ranked.csv.gz"),
            compression="gzip",
        )
    else:
        print(dft.head())
        dft.to_csv(
            expanduser(f"~/data/wos/comm_metrics/{mt}_metric_{ms}_ranked_tmp.csv.gz"),
            compression="gzip",
        )
