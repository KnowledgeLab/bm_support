...
def set_closest_max(x):
    ...
    delta = x_up / xm - 1
    print(
        "In set_closest_max() round_ind {}, delta {}, x_up {}, xm {}".format(
            round_ind, delta, x_up, xm
        )
    )
    if delta < 0.2:
...
def get_dists(beta_data_df, foo, gridn=100, alpha0=0.5, beta0=0.5, verbose=False):
    ...
    for j, item in beta_data_df.iterrows():
        ...
        if k == prev_item[1] and n == prev_item[2]:
            ...
            pdfs = [
                beta(alpha0 + k, beta0 + n - k).pdf,
                beta(alpha0 + k2, beta0 + n2 - k2).pdf,
            ]
            dist = foo(*pdfs, grid)
            if verbose:
                print(
                    "{0}/{1}:  d (pos, ambi) = {2:.3f}".format(
                        j, beta_data_df.shape[0], dist
                    )
                )
        dists.append(dist)
...
for k, df in df_dict.items():
    ...
    # print(df_info2)
    dfr = pd.merge(
        df_info[["thr", "dist", "k", "n"]],
        df_info2[["thr", "dist", "k", "n"]],
        on="thr",
        suffixes=["_pos", "_neg"],
    )
    # print(dfr)
