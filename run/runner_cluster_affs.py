...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--sourcepath", default="~/data/wos/", help="path to input data"
    )

    parser.add_argument("-d", "--destpath", default=".", help="path to output data")

    parser.add_argument("-n", "--nproc", default=1, type=int, help="number of threads")

    parser.add_argument(
        "--pm-fname",
        default="pmids.csv.gz",
    ...
    pm2c, aff_dict = cluster_affiliations(
        df_wos, pmids, n_processes=args.nproc, debug=True
    )
