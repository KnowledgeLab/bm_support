...
for target in targets:
    ...
    for k, df in df_dict.items():
        ...
        for s in selectors:
            ...
            case_features = [
                c for c in case_features if not any([x in c for x in excl_symbol_list])
            ]
            if exec_mode == "neutral" or exec_mode == "posneg":
            ...
            cr_abs, cr = get_corrs(
                df, target, case_features, -1, dropnas=False, individual_na=True
            )
            cr = -cr
...
for s, item in example_corr.items():
    ...
    if s == "claim":
        ...
        good_index_tr = [x.replace("rcommrel", "rrelcomm") for x in good_index]
        aux = [x.split("_") for x in good_index_tr]
        aux = [
            x[1:3] + x[0:1] + x[3:]
            if any(["affind" in y for y in x])
            or any(["rcomm" in y for y in x])
            or any(["rrelcomm" in y for y in x])
            or any(["count" in y for y in x])
            else x
            for x in aux
        ]
        ...
        good_index = [
            x
            for x in good_index
            if (not "litgw_comm" in x) or ("litgw_comm" in x and "dyn" in x)
        ]
        aux = [x.split("_") for x in good_index]
        aux = [x[:2] + ["".join(x[6:])] + x[2:6] for x in aux]
        aux = [
            x[1:2] + x[0:1] + x[2:]
            if any(["rncomms" in y for y in x]) or any(["suppind" in y for y in x])
            else x
            for x in aux
        ]
        aux = list(zip(good_index, aux))
...
for target in targets[:]:
    ...
    for mode in selectors[:]:
        ...
        ykeys = sorted([c for c in ykeys if "t0" in c]) + sorted(
            [c for c in ykeys if "t0" not in c]
        )
        ...
        for ttype in control:
            ...
            dump_file_fname = expanduser(
                f"~/data/kl/corrs/corrs_binary_{target}_{exec_mode}_"
            ...
            fig_fname = expanduser(
                f"~/data/kl/figs/corr/heatmap_corr_{target}_{exec_mode}_"
