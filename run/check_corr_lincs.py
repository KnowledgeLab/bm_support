...
def get_id_up_dn_df(origin, n, a, b, version):
    ...
    prefix_str = "pairs_freq_{0}_v_{1}_n_{2}_a_{3}_b_{4}.csv.gz".format(
        origin, version, n, a, b
    )
    df = pd.read_csv(
        join(expanduser("~/data/kl/claims/"), prefix_str),
        compression="gzip",
        index_col=2,
    )
    return df
...
def get_up_dn_report(origin, version, datatype, batchsize, n, a, b, func, case):
    ...
    df_merged = pd.merge(
        df_pairs[o_columns], reports_df, right_index=True, left_index=True, how="right"
    )
    return df_merged
...
def get_lincs_df(origin, version, n, a, b):
    ...
    df = pd.read_csv(
        join(
            fpath,
            "lincs_{0}_v_{1}_n_{2}_a_{3}_b_{4}.csv.gz".format(origin, version, n, a, b),
        ),
        compression="gzip",
    )
    return df
...
def process(args_reports_list, args_lincs_list):
    ...
    for args, dfl in args_lincs_list:
        ...
        acc2 = (
            ("96 h", "120 h", "24 h", "1 h", "4 h"),
            ("MCF7", "PC3", "A375", "VCAP", "HA1E"),
            ("2 µL", "1 µL", "1.5 µL", "100 ng/µL"),
            [("trt_oe")],
            (1, 0),
        )
        ...
        for combo in combos2:
            ...
            if s > 10:
                ...
                dfl3 = (
                    ...
                    .apply(lambda x: x["score"].mean())
                    .reset_index()
                )
                for args, df in reps:
                    df_cmp = pd.merge(
                        df[[up, dn, "freq", "pi_last"]],
                        dfl3[[up, dn, 0]],
                        how="inner",
                        on=o_columns,
                    )
                    ...
                    res_dict.update(
                        dict(
                            zip(
                                ["size", "cov_freq", "cov_model"],
                                [s, cov_freq_, cov_flat_],
                            )
                        )
                    )
...
def process_simple(args_reports_list, args_lincs_list):
    ...
    for args, dfl in args_lincs_list:
        ...
        for args, df in reps:
            ...
            res_dict.update(
                dict(
                    zip(["size", "cov_freq", "cov_model"], [size, cov_freq_, cov_flat_])
                )
            )
            results.append(res_dict)
    return results
...
invariant_args = {"origin": origin, "a": a, "b": b}
